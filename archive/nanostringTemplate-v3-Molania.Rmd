...
nanostringData <- processNanostringData(file.folder,
                             ...
                             normalization = "nCounter",
                             bgType = "t.test", bgPVal = 0.001,
                             housekeeping = hk.genes,
                             includeQC = TRUE)

# Try NanoStringDiff
#test <- createNanoStringSetFromEset(nanostringData)

# Test RUV
nanostringDataRUV <- processNanostringData(file.folder,
                                        sampleTab = sampleTab, idCol = "SampleNames",  groupCol = "Tissues", replicateCol = "Patient.barcodes",
                             normalization = "RUV")
...
# Try using CPDB
genesetResults2 <- mHG.genesets.multi(limmaResults, geneset.file = "../../Pathway_DBs/ConsensusPathDB/CPDB_pathways_genes_symbol.tab",
                                      sourceDB = "Reactome",
                               t_thresh = 2, numReq_overall = 10, numReq_inSet = 5)

# Try with FGSEA
genesetResults2f <- limmaToFGSEA(limmaResults, gene.sets = "../../Pathway_DBs/ConsensusPathDB/CPDB_pathways_genes_symbol.tab", sourceDB = "Reactome",
                                  min.set = 5, rank.by = "t", skip.first = TRUE)



genesetResults3 <- mHG.genesets.multi(limmaResults, geneset.file = "../../Pathway_DBs/ConsensusPathDB/CPDB_pathways_genes_symbol.tab",
                               t_thresh = 2, numReq_overall = 10, numReq_inSet = 5)

```
```{r compareGSEA}

compareGSEA <- merge(genesetResults2$`normal-lung`$negative, genesetResults2f$`normal-lung`,
                     by.x = "row.names", by.y = "pathway")

```


```{r gseaPost}
ledge <- fgseaToLEdge(genesetResults2f)

ledge <- fgseaToLEdge(genesetResults2f, cutoff.type = "none")

ledge <- fgseaToLEdge(genesetResults2f, cutoff.type = "pval",
                      cutoff = 2)

ledge <- fgseaToLEdge(genesetResults2f, cutoff.type = "NES",
                      cutoff = 2)
ledge <- fgseaToLEdge(genesetResults2f, cutoff.type = "NES",
                      cutoff = 2, nes.abs.cutoff = FALSE)
ledge <- fgseaToLEdge(genesetResults2f, cutoff.type = "NES",
                      cutoff = -2, nes.abs.cutoff = FALSE)
```

```{r compareNorm}
all(rownames(nanostringData) == rownames(nanostringDataRUV))
all(colnames(nanostringData) == colnames(nanostringDataRUV))

compare.norm <- data.frame(nCounter = log2(as.vector(exprs(nanostringData))),
                           RUV = as.vector(exprs(nanostringDataRUV)),
                           CodeClass = rep(fData(nanostringData)$CodeClass, times = ncol(nanostringData)))
ggplot(compare.norm, aes(x=nCounter, y=RUV, colour = CodeClass)) +
...
```{r compare Limma}
limmaRUV <- runLimmaAnalysis(nanostringDataRUV, base.group = NULL)

qplot(limmaResults$coefficients[,2], limmaRUV$coefficients[,2])
#cor.test(limmaResults$coefficients[,2], limmaRUV$coefficients[,2], method = "spearman")

```
```{r}
qplot(limmaResults$p.value[,2], limmaRUV$p.value[,2])
...
genesetRUV <- mHG.genesets.multi(limmaRUV, geneset.file,
                               t_thresh = 2, numReq_overall = 10, numReq_inSet = 5)

qplot(genesetResults$`normal-lung`$negative$p.val, 
      genesetRUV$`normal-lung`$negative$p.val)

table(genesetResults$`normal-lung`$negative$p.val < 0.05, genesetRUV$`normal-lung`$negative$p.val < 0.05)
...
# Pathway files
leadingEdge <- mHG.to.LEdge(genesetResults, limmaResults, 
                                  t.thresh = 2, q.thresh = 0.2, geneset.file)
mHG.grouped <- groupByDistance(genesetResults2$`normal-lung`$negative, leadingEdge$`normal-lung`, 
                               join.threshold = 0.5, returns = "all")

mHG.postprocessing(genesetResults2, leadingEdge, limmaResults, 
                     join.threshold = 0.5, reportDir="data/Molania/NanostringNormalization-master/Example 1_LungCancerStudy/test/txt")
...
```{r postProcessFGSEA, include = FALSE}

gs.ledge <- fgseaToLEdge(genesetResults2f, cutoff.type = "padj", cutoff = 0.05)
gs.group <- groupFGSEA(genesetResults2f$`normal-lung`, l.edge = gs.ledge$`normal-lung`, 
                       join.threshold = 0.5, returns = "all")

groupedGSEAtoStackedReport(gs.group, gs.ledge$`normal-lung`, limmaResults,
                                   outputDir = "test")

```


### Comparing mutations among cancer

```{r, include = FALSE}
nanostringData <- processNanostringData(file.folder,
                                        sampleTab = sampleTab, idCol = "SampleNames",  groupCol = "Mutation", replicateCol = "Patient.barcodes",
                             normalization = "nCounter",
                             bgType = "t.test", bgPVal = 0.001,
                             housekeeping = hk.genes,
                             includeQC = TRUE)

# Only Cancer, remove ALK-mut (only 1 sample)
nanostringData <- nanostringData[,nanostringData$Mutation != "ALK" & 
                                 nanostringData$Tissues == "Cancer"]

limmaResults <- runLimmaAnalysis(nanostringData, base.group = "NIL")
#makeDiffExprFile(limmaResults, filename = NULL)
...
ledge2f <- fgseaToLEdge(genesetResults2f, cutoff.type = "pval", cutoff = 0.05)
testTab <- makeFGSEAmasterTable(genesetResults2f, ledge2f,
                                 join.threshold = 0.5,
                                 ngroups = NULL,
                                 dist.method = "binary",
                                 filename = NULL)

fgseaPostprocessing(genesetResults2f, ledge2f, limmaResults, 
                    join.threshold = 0.5, reportDir = "test/fgseaPP")
fgseaPostprocessingXLSX(genesetResults2f, ledge2f, limmaResults, 
                    join.threshold = 0.5, filename = "test/fgseaPP.xlsx")


ledge2f <- fgseaToLEdge(genesetResults2f, cutoff.type = "padj", cutoff = 0.05)
testTab <- makeFGSEAmasterTable(genesetResults2f, ledge2f,
                                 join.threshold = 0.5,
                                 ngroups = NULL,
                                 dist.method = "binary",
