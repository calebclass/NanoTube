...
renderPlotly({
  ...
  #Cluster genes
  genes <- labels(as.dendrogram(hclust(dist(dat.scaled[rownames(dat.scaled) %in% rownames(annot_row.df),]), method="ward.D")))
  ...
  pHM <- plot_ly(data = dat.hm, x = ~X2, y = ~X1, z = ~value, 
                  type = "heatmap", colors = colorRamp(c("blue", "white", "red")), alpha = 1) %>%
          layout(xaxis = ax, yaxis = ax)
  pCB <- plot_ly(data = annot_row.df, x = ~X2, y = ~X1, z = ~value,
                 type = "heatmap", colors = colorRamp(c("grey95", "black")), alpha = 1) %>%
          layout(xaxis = ax, yaxis = ax)
  subplot(pCB, pHM, nrows = 1, widths = c(ncol(annot_row) / (ncol(annot_row)+ncol(dat.scaled)), 1))
