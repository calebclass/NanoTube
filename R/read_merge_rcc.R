...
read_merge_rcc <- function(fileList, includeQC = FALSE, logfile = "") {
  ...
  if (length(unique(lapply(dat, function(df) df$exprs$Name))) > 1) {
    stop(cat("\nRCC files do not have same gene names. Stopping...", file=logfile, append=TRUE))
  }
  cat("\nChecked gene name consistency in .RCC files.", file=logfile, append=TRUE)

  merged.dat <- list(exprs = sapply(dat, function(X) X[[1]][,4]),
                     dict = dat[[1]][[1]][,1:3])

  # We use Accessions for row names, but it's possible for those to be duplicated.
  # To account for this, use the `make.names` function.
  rownames(merged.dat[[1]]) <- rownames(merged.dat[[2]]) <- make.names(merged.dat$dict[,3], unique = TRUE)
  colnames(merged.dat[[1]]) <- gsub(".*\\/", "", fileList)

  if (any(merged.dat$dict$CodeClass == "Housekeeping")) {
    cat("\n", paste(sum(merged.dat$dict$CodeClass == "Housekeeping")), " genes labeled as 'housekeeping'.",
        file=logfile, sep = "", append = TRUE)
  } else {
    warning(cat("\nNo genes labeled as 'housekeeping' in data set.", file=logfile, append=TRUE))
  }
