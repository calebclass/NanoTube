...
#'
#' This function clusters GSEA results by leading edge similarity, and then combines to a
...
makeFGSEAmasterTable <- function(genesetResults, leadingEdge,
  ...
  for (i in names(genesetResults)) {
    ...
    if (is.null(leadingEdge[[i]])) {
      ...
      geneset.clustered[[i]] <- groupFGSEA(genesetResults[[i]], leadingEdge[[i]],
                                           join.threshold, ngroups,
                                           dist.method, returns="all")
    }
    
    colnames(geneset.clustered[[i]])[-1] <- paste0(colnames(geneset.clustered[[i]])[-1], " (", i, ")")
  }

  dat.xpt <- Reduce(function(...) merge(..., all=TRUE, by="pathway"), geneset.clustered)
