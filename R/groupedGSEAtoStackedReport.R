...
groupedGSEAtoStackedReport <- function(grouped.gsea, leadingEdge, de.fit,
  ...
  grouped.gsea <- grouped.gsea[rownames(grouped.gsea) %in% colnames(leadingEdge),]
  
  xpt.mat.full <- merge(diffExpr.mat, leadingEdge[,as.character(rownames(grouped.gsea))], by="row.names", all = FALSE)
  ...
  if (ncol(leadingEdge) == 1) {
    ...
    grouped.gsea <- cbind(as.character(rownames(grouped.gsea)), grouped.gsea)
    colnames(grouped.gsea)[1] <- 'gene set'
    
    for (i in 1:max(grouped.gsea$Cluster)) {
