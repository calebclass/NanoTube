...
#' sets ('none')
#' @param cutoff Numeric cutoff for filtering (not used if cutoff.type == "none")
...
fgseaToLEdge <- function(fgsea.res, cutoff.type = c("padj", "pval", "NES", "none"),
  ...
  for (i in names(fgsea.res)) {
    ...
    if (cutoff.type %in% c("padj", "pval")) {
      fgsea.sub <- fgsea.res[[i]][as.vector(fgsea.res[[i]][,cutoff.type] < cutoff & 
    ...
    if (nrow(fgsea.sub) == 0) {
      ...
      ledge.res[[i]] <- matrix(0, nrow = length(ledge.genes), ncol = nrow(fgsea.sub),
                               dimnames = list(ledge.genes, fgsea.sub$pathway))
      
      for (j in 1:nrow(fgsea.sub)) ledge.res[[i]][ledge.dat[[j]],j] <- 1
    }
  }
