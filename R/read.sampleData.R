...
#'
#' @param dat expression data, read in by read.merge.rcc or read.delim
...
read.sampleData <- function(dat, file.name, idCol = NULL, groupCol, replicateCol = NULL) {
  # Read in the file
  ...
  if (ncol(dat$exprs) == nrow(sampDat)) {
    if (all(rownames(sampDat) %in% colnames(dat$exprs))) {
      # If both files have same sample names, ensure that samples are in the 
      # same order.
      dat$samples <- sampDat[colnames(dat$exprs),]
    } else {
      # If files have different sample names (or no sample names in meta file),
      # assume that they are in the same order (and warn).
      dat$samples <- sampDat
